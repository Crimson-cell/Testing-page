...
export const postCursorActivity = (payload) => (dispatch) => {
    return firestore.collection('cursorEvents').where('user_id', '==', payload[1]).get()
    .then((snapshot) => {
        snapshot.docs[0].ref.update({events: firebasestore.FieldValue.arrayUnion(payload[0])})
        .then(() => {
...
export const newCursorActivity = (user) => (dispatch) => {
    return firestore.collection('cursorEvents').add({...user})
    .then(docRef => {
